(function () {
    var ProjectsController = function ($scope, projectService, $log, $routeParams, $location) {
        ...
        function convertDateToBindable(date) {
            ...
            return dateObj.getFullYear() + "-" + getTwoDigitString(dateObj.getMonth() + 1) + "-" + getTwoDigitString(dateObj
                .getDate());
        }
        function getTwoigitDateString(number) {
        ...
        $scope.insertProject = function (project) {
            projectService.insertProject(project).then(function (data) {
                console.log(data);
        ...
        $scope.modifyProject = function (existingProject) {
            ...
            projectService.modifyProject(existingProject).then(function () {
                $location.path("/Projects");
            }, errorDetails);
        };
        $scope.deleteProject = function (project) {
            $log.info(project)
            projectService.deleteProject(project).then(projects, errorDetails);
        };
